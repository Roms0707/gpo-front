...
serve(async (req)=>{
  ...
  try {
    const { tournament_id, user_id, team_id } = await req.json();
    console.log(`[Tournament Registration Validation] Request for tournament: ${tournament_id}, user: ${user_id}, team: ${team_id}`);
    // Validate input
    ...
    const { data: existingRegistration, error: regCheckError } = await supabase.from('tournament_registrations').select('id').eq('tournament_id', tournament_id).eq('user_id', user_id).maybeSingle();
    if (regCheckError) {
      return new Response(JSON.stringify({
        success: false,
    ...
    if (isTeamTournament && tournament.max_players_per_team) {
      ...
      if (team_id) {
        const { count: teamMemberCount, error: teamCountError } = await supabase.from('team_members').select('*', {
          count: 'exact',
          head: true
        }).eq('team_id', team_id).eq('status', 'accepted');
        if (teamCountError) {
          return new Response(JSON.stringify({
            success: false,
            error: "Error checking team capacity"
          }), {
            status: 500,
            headers: {
              "Content-Type": "application/json",
    ...
    if (tournament.max_nb_players) {
      ...
      if (isTeamTournament) {
        // Count unique teams
        const { count: teamCount, error: teamCountError } = await supabase.from('tournament_registrations').select('team_id', {
          count: 'exact',
          head: true
        }).eq('tournament_id', tournament_id).not('team_id', 'is', null).in('status', [
          'pending',
          'approved'
        ]);
        if (teamCountError) {
          return new Response(JSON.stringify({
            success: false,
            error: "Error checking tournament capacity"
          }), {
            status: 500,
            headers: {
              "Content-Type": "application/json",
        ...
        const { count: participantCount, error: participantCountError } = await supabase.from('tournament_registrations').select('*', {
          count: 'exact',
          head: true
        }).eq('tournament_id', tournament_id).in('status', [
          'pending',
          'approved'
        ]);
    ...
    if (tournament.eligible_countries && user.country) {
      ...
      if (!eligibleCountries.includes(user.country)) {
        return new Response(JSON.stringify({
          success: false,
          error: "Your country is not eligible for this tournament"
        }), {
          status: 400,
          headers: {
            "Content-Type": "application/json",
    ...
    if (tournament.minimum_age && user.date_of_birth) {
      ...
      if (age < tournament.minimum_age) {
        return new Response(JSON.stringify({
          success: false,
          error: `You must be at least ${tournament.minimum_age} years old to participate`
        }), {
          status: 400,
          headers: {
            "Content-Type": "application/json",
