...
const getValorantRegion = (region)=>{
  const regionMap = {
    'euw1': 'eu',
    'eun1': 'eu',
    'na1': 'na',
...
serve(async (req)=>{
  ...
  try {
    const { puuid, region = 'eu', count = 5 } = await req.json();
    ...
    const matchIds = matchListData.history?.map((match)=>match.matchId) || [];
    ...
    for (const matchId of matchIds.slice(0, count)){
      try {
        ...
        const playerData = matchData.players?.find((p)=>p.puuid === puuid);
        if (!playerData) {
          console.warn(`[Valorant Match History] Player not found in match ${matchId}`);
          continue;
        }
        // Get teammates (same team as the player)
        const teammates = matchData.players?.filter((p)=>p.puuid !== puuid && p.teamId === playerData.teamId)?.slice(0, 4) // Limit to 4 teammates
        ?.map((p)=>({
            puuid: p.puuid,
        ...
        const roundResults = matchData.roundResults?.map((round, index)=>({
            roundNum: index + 1,
            roundResult: round.roundResult,
            roundCeremony: round.roundCeremony || 'none'
          })) || [];
        // Determine if the player won
        const playerTeam = matchData.teams?.find((team)=>team.teamId === playerData.teamId);
