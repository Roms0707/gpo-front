...
interface RiotIdRequest {
  gameName: string;
  tagline: string;
  region?: string;
}

interface RiotIdResponse {
  valid: boolean;
  puuid?: string;
  error?: string;
  summonerInfo?: any; // Add this for summoner data
  rankedStats?: any;  // Add this for ranked stats
}
...
serve(async (req) => {
  ...
  try {
    const { gameName, tagline, region = 'euw1' } = await req.json() as RiotIdRequest;
    ...
    if (!gameName || !tagline) {
      ...
      return new Response(
        JSON.stringify({ valid: false, error: "Game name and tagline are required" }),
    ...
    if (dbError || !gameApiIntegration || !gameApiIntegration.api_key) {
      ...
      return new Response(
        JSON.stringify({ valid: false, error: "Riot API key not configured" }),
    ...
    if (!accountResponse.ok) {
      ...
      if (accountResponse.status === 404) {
        return new Response(
          JSON.stringify({ valid: false, error: "Riot ID not found" }),
          { status: 200, headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" } }
        );
      }
      return new Response(
        JSON.stringify({ valid: false, error: `Validation failed: ${accountResponse.status} - ${errorText}` }), // IMPROVED ERROR MESSAGE
        { status: 200, headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" } }
      );
    }
