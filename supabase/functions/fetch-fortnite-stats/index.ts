...
serve(async (req)=>{
  ...
  try {
    const { playerIdentifier, platform = 'epic' } = await req.json();
    ...
    if (!playerIdentifier) {
      ...
      return new Response(JSON.stringify({
        success: false,
        error: "Player identifier is required"
      }), {
        status: 400,
        headers: {
    ...
    const { data: gameApiIntegration, error: dbError } = await supabase.from('game_api_integrations').select('api_key, api_url').eq('api_name', 'Fortnite API').eq('is_active', true).single();
    if (dbError || !gameApiIntegration) {
      ...
      return new Response(JSON.stringify({
        success: false,
        error: "Fortnite stats coming soon! This feature is currently being developed."
      }), {
        status: 500,
        headers: {
          "Content-Type": "application/json",
    ...
    try {
      ...
      return new Response(JSON.stringify({
        success: false,
        error: "Invalid API key format in database configuration"
      }), {
        status: 500,
        headers: {
          "Content-Type": "application/json",
    ...
    if (!api_key) {
      ...
      return new Response(JSON.stringify({
        success: false,
        error: "API key missing from database configuration"
      }), {
        status: 500,
        headers: {
          "Content-Type": "application/json",
