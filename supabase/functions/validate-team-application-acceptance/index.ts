...
serve(async (req)=>{
  ...
  try {
    const { application_id, team_id, user_id } = await req.json();
    // Validate input
    if (!application_id || !team_id || !user_id) {
      return new Response(JSON.stringify({
        success: false,
        error: "Application ID, Team ID, and User ID are required"
      }), {
        status: 400,
        headers: {
    ...
    const { data: application, error: applicationError } = await supabase.from('team_applications').select(`
        id,
        status,
        team_id,
        user_id,
        tournament_id
      `).eq('id', application_id).eq('team_id', team_id).eq('user_id', user_id).single();
    ...
    const { data: team, error: teamError } = await supabase.from('teams').select(`
        id,
        name,
        tournament_id,
    ...
    const { data: existingMembership, error: membershipError } = await supabase.from('team_members').select(`
        id,
        team_id,
        teams:team_id (
          tournament_id
    ...
    if (membershipError) {
      return new Response(JSON.stringify({
        success: false,
        error: "Error checking user's team membership"
      }), {
        status: 500,
        headers: {
          "Content-Type": "application/json",
    ...
    const existingTeamForTournament = existingMembership?.find((membership)=>membership.teams?.tournament_id === team.tournament_id);
    if (existingTeamForTournament) {
      return new Response(JSON.stringify({
        success: false,
        error: "User is already a member of another team for this tournament"
      }), {
        status: 400,
        headers: {
          "Content-Type": "application/json",
    ...
    if (maxPlayersPerTeam) {
      const { count: currentMemberCount, error: countError } = await supabase.from('team_members').select('*', {
        count: 'exact',
        head: true
    ...
    if (tournament_data?.eligible_countries && user.country) {
      ...
      if (!eligibleCountries.includes(user.country)) {
        return new Response(JSON.stringify({
          success: false,
          error: "User's country is not eligible for this tournament"
        }), {
          status: 400,
          headers: {
            "Content-Type": "application/json",
    ...
    if (tournament_data?.minimum_age && user.date_of_birth) {
      ...
      if (age < tournament_data.minimum_age) {
        return new Response(JSON.stringify({
          success: false,
          error: `User must be at least ${tournament_data.minimum_age} years old`
        }), {
          status: 400,
          headers: {
            "Content-Type": "application/json",
