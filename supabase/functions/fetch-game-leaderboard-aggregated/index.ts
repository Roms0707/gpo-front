...
serve(async (req)=>{
  ...
  try {
    const { game_id, leaderboard_type = 'both', search_query = '', rank_tier = 'All Ranks', sort_field = 'elo_rating', sort_direction = 'desc', limit = 50, offset = 0 } = await req.json();
    ...
    // 1. Get game details
    const { data: gameData, error: gameError } = await supabase.from('games').select('id, name, publisher, image_url').eq('id', game_id).single();
    ...
    if (leaderboard_type === 'solo' || leaderboard_type === 'both') {
      let soloQuery = supabase.from('player_rankings').select(`
          id,
          user_id,
          elo_rating,
          wins,
      ...
      soloQuery = soloQuery.order(sort_field, {
        ascending
      });
      // Apply pagination
      ...
      if (soloError) {
        ...
        soloLeaderboard = soloData.map((ranking, index)=>{
          ...
          return {
            ...
            tournaments_played: Math.floor(matches / 3),
            wins: ranking.wins,
            losses: ranking.losses,
            matches: matches,
            top_5: Math.floor(ranking.wins * 0.6),
            win_rate: winRate,
    ...
    if (leaderboard_type === 'team' || leaderboard_type === 'both') {
      let teamQuery = supabase.from('team_rankings').select(`
          id,
          team_id,
          elo_rating,
          wins,
      ...
      // Apply sorting
      const ascending = sort_direction === 'asc';
      teamQuery = teamQuery.order(sort_field, {
        ascending
      });
      // Apply pagination
    ...
    const { data: matchResults, error: matchError } = await supabase.from('match_results').select(`
        id,
        match_date,
        is_team_match,
        winner_player_id,
        ...
        elo_change,
        tournament_id
      `).eq('game_id', game_id).order('match_date', {
      ascending: false
    }).limit(5);
    let recentMatches = [];
    if (!matchError && matchResults && matchResults.length > 0) {
      // Process match results to get player/team names
      recentMatches = await Promise.all(matchResults.map(async (match)=>{
        ...
        if (match.is_team_match) {
          ...
          if (match.winner_team_id) {
            const { data: winnerTeam } = await supabase.from('teams').select('name').eq('id', match.winner_team_id).single();
            if (winnerTeam) {
          ...
          if (match.loser_team_id) {
            const { data: loserTeam } = await supabase.from('teams').select('name').eq('id', match.loser_team_id).single();
            if (loserTeam) {
          ...
          if (match.winner_player_id) {
            const { data: winnerUser } = await supabase.from('users').select('username').eq('id', match.winner_player_id).single();
            if (winnerUser) {
          ...
          if (match.loser_player_id) {
            const { data: loserUser } = await supabase.from('users').select('username').eq('id', match.loser_player_id).single();
            if (loserUser) {
