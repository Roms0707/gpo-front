...
serve(async (req)=>{
  ...
  try {
    const { steamId64, includeBans = false, includeGames = false, includeLevel = false, includeGameStats = false, gameAppIds = [] } = await req.json();
    ...
    const { data: gameApiIntegration, error: dbError } = await supabase.from('game_api_integrations').select('api_key').eq('api_name', 'Steam Web API').eq('is_active', true).single();
    ...
    if (!profileResponse.ok) {
      ...
      return new Response(JSON.stringify({
        success: false,
        error: `Failed to fetch Steam profile: ${profileResponse.status}`
      }), {
        status: 200,
        headers: {
          "Content-Type": "application/json",
      ...
      try {
        ...
        if (gamesResponse.ok) {
          ...
          if (gamesResponseData.response && gamesResponseData.response.games) {
            ...
            const sortedGames = gamesResponseData.response.games.sort((a, b)=>(b.playtime_forever || 0) - (a.playtime_forever || 0)).slice(0, 10);
            gamesData = {
    ...
    if (includeGameStats && gameAppIds.length > 0) {
      ...
      gameStatsData = [];
      for (const appId of gameAppIds.slice(0, 5)){
        try {
