...
serve(async (req)=>{
  ...
  try {
    const { gameId, playerIdentifier, platform } = await req.json();
    ...
    if (!gameId || !playerIdentifier || !platform) {
      ...
      return new Response(JSON.stringify({
        success: false,
        error: "Game ID, player identifier, and platform are required"
      }), {
        status: 400,
        headers: {
    ...
    const { data: gameApiIntegration, error: dbError } = await supabase.from('game_api_integrations').select('api_key, api_url').eq('game_id', gameId).eq('api_name', 'Tracker.gg API').eq('is_active', true).single();
    if (dbError || !gameApiIntegration) {
      ...
      return new Response(JSON.stringify({
        success: false,
        error: "Tracker.gg API configuration not found"
      }), {
        status: 500,
        headers: {
          "Content-Type": "application/json",
    ...
    if (!gameApiIntegration.api_key || !gameApiIntegration.api_url) {
      ...
      return new Response(JSON.stringify({
        success: false,
        error: "Tracker.gg API key or URL not configured"
      }), {
        status: 500,
        headers: {
          "Content-Type": "application/json",
