...
const validateDiscordHandle = (handle)=>{
  ...
  if (!discordRegex.test(handle)) {
    return {
      valid: false,
      error: "Invalid Discord handle format. Use 'username' or 'username#1234'"
    };
  }
  // Check for offensive content
  if (containsOffensiveContent(handle)) {
    return {
      valid: false,
      error: "Discord handle contains inappropriate content"
    };
  }
  return {
    valid: true
...
const validateTwitterHandle = (handle)=>{
  ...
  if (cleanHandle.length < 1) {
    return {
      valid: false,
      error: "Twitter/Twitch handle cannot be empty"
    };
  }
  if (cleanHandle.length > 15) {
    return {
      valid: false,
      error: "Twitter/Twitch handle must be no more than 15 characters long"
    };
  }
  ...
  if (!twitterRegex.test(cleanHandle)) {
    return {
      valid: false,
      error: "Twitter/Twitch handle can only contain letters, numbers, and underscores"
    };
  }
  // Check for offensive content
  if (containsOffensiveContent(cleanHandle)) {
    return {
      valid: false,
      error: "Twitter/Twitch handle contains inappropriate content"
    };
  }
  return {
    valid: true
...
const validateCountryCode = (countryCode)=>{
  ...
  if (!/^[A-Z]{2}$/.test(countryCode)) {
    return {
      valid: false,
      error: "Country code must be a valid 2-letter ISO code"
    };
  }
...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: user, error: userError } = await supabase.from('users').select('id, username').eq('id', user_id).single();
    console.log("[User Profile Validation] User query result:", {
      user,
      userError
    });
    if (userError || !user) {
    ...
    if (username !== undefined && username !== null) {
      ...
      if (trimmedUsername !== user.username) {
        ...
        const { data: existingUser, error: existingUserError } = await supabase.from('users').select('id').eq('username', trimmedUsername).neq('id', user_id).maybeSingle();
        console.log("[User Profile Validation] Username availability check result:", {
          existingUser,
          existingUserError
        });
    ...
    // 6. Validate country if provided
    if (country !== undefined && country !== null) {
      console.log("[User Profile Validation] Validating country:", country);
      const trimmedCountry = country.trim();
      if (trimmedCountry.length > 0) {
        console.log("[User Profile Validation] Calling validateCountryCode");
        const countryValidation = validateCountryCode(trimmedCountry);
        console.log("[User Profile Validation] Country validation result:", countryValidation);
        if (!countryValidation.valid) {
          console.error("[User Profile Validation] Country validation failed:", countryValidation.error);
          return new Response(JSON.stringify({
            success: false,
      ...
      console.log("[User Profile Validation] Country validation passed");
    }
