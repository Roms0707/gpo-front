...
serve(async (req)=>{
  ...
  try {
    ...
    // Parse request body to get config_id
    let configId = 'default'; // default fallback
    try {
      if (req.body) {
        const requestBody = await req.json();
        configId = requestBody.config_id || 'default';
      }
      console.log(`[Galaxy Sync] Using config_id: ${configId}`);
    } catch (parseError) {
      console.log('[Galaxy Sync] No config_id provided in request body, using default');
    }
    // Fetch project configuration for product_id and campaign_id
    let projectConfig;
    try {
      console.log(`[Galaxy Sync] Fetching project configuration for config_id: ${configId}`);
      const { data: configData, error: configError } = await supabase.from('project_configurations').select('config_id, product_id, campaign_id, is_active').eq('config_id', configId).eq('is_active', true).single();
      if (configError || !configData) {
        console.error(`[Galaxy Sync] Error fetching project config:`, configError?.message || 'Config not found');
        throw new Error(`Project configuration not found for config_id: ${configId}`);
      }
      projectConfig = configData;
      console.log(`[Galaxy Sync] Project config loaded successfully:`, {
        config_id: projectConfig.config_id,
        product_id: projectConfig.product_id,
        campaign_id: projectConfig.campaign_id
      });
    } catch (err) {
      console.error(`[Galaxy Sync] Critical error loading project config:`, err.message, err.stack);
      return new Response(JSON.stringify({
        success: false,
        error: `Failed to load project configuration: ${err.message}`
      }), {
        status: 500,
        headers: {
    ...
    // Fetch Galaxy API authentication configuration from the database
    let galaxyApiConfig;
    try {
      console.log("[Galaxy Sync] Fetching Galaxy API authentication configuration from database...");
      const { data, error } = await supabase.from('platform_api_integrations').select('api_key, api_url').eq('api_name', 'Galaxy API').eq('is_active', true).single();
      if (error || !data) {
      ...
      console.log("[Galaxy Sync] Galaxy API authentication config fetched from DB successfully");
    } catch (err) {
      console.error("[Galaxy Sync] Critical error during authentication config fetch:", err.message, err.stack);
      return new Response(JSON.stringify({
        success: false,
        error: `Failed to load Galaxy API config: ${err.message}`
      }), {
        status: 500,
        headers: {
          "Content-Type": "application/json",
    ...
    try {
      ...
      return new Response(JSON.stringify({
        success: false,
        error: `Invalid Galaxy API credentials format: ${parseError.message}`
      }), {
        status: 500,
        headers: {
          "Content-Type": "application/json",
    ...
    // Build galaxy config with OVERRIDDEN values from project_configurations
    const galaxyConfig = {
    ...
    console.log(`[Galaxy Sync] ========================================`);
    console.log(`[Galaxy Sync] Configuration Override Applied:`);
    console.log(`[Galaxy Sync] - config_id: ${configId}`);
    console.log(`[Galaxy Sync] - campaign_id: ${galaxyConfig.campaign_id} (from project_configurations)`);
    console.log(`[Galaxy Sync] - product_id: ${galaxyConfig.product_id} (from project_configurations)`);
    console.log(`[Galaxy Sync] ========================================`);
    // Validate essential credentials
    if (!galaxyConfig.api_key || !galaxyConfig.api_secret_key || !galaxyConfig.campaign_id || !galaxyConfig.service_id || !galaxyConfig.country_code || !galaxyConfig.language_code || !galaxyConfig.base_url || !galaxyConfig.product_id) {
      console.error("[Galaxy Sync] Missing essential Galaxy API credentials after override.");
      console.error("[Galaxy Sync] Current config:", {
        has_api_key: !!galaxyConfig.api_key,
        has_api_secret_key: !!galaxyConfig.api_secret_key,
        has_campaign_id: !!galaxyConfig.campaign_id,
        has_product_id: !!galaxyConfig.product_id,
        has_service_id: !!galaxyConfig.service_id,
        has_country_code: !!galaxyConfig.country_code,
        has_language_code: !!galaxyConfig.language_code,
        has_base_url: !!galaxyConfig.base_url
      });
      return new Response(JSON.stringify({
        success: false,
        error: "Missing essential Galaxy API credentials."
      }), {
        status: 500,
        headers: {
          "Content-Type": "application/json",
    ...
    const getVideoUrl = (content)=>{
      ...
      const isHlsUrl = (url)=>{
        return url.toLowerCase().includes('.m3u8') || url.toLowerCase().includes('manifest') || url.toLowerCase().includes('playlist');
      };
      ...
      if (Array.isArray(content.deliveries?.additionalDeliveries) && content.deliveries.additionalDeliveries.length > 0) {
    ...
    const getCoverImageUrl = (content)=>{
      if (!content.assets?.cover || content.assets.cover.length === 0) {
        return '';
      }
      // Prefer landscape images for video thumbnails
      const landscapeImage = content.assets.cover.find((cover)=>cover.ratio_tech_label === 'landscape-16-9');
      if (landscapeImage) {
    ...
    try {
      ...
      const { data, error } = await supabase.from('galaxy_rubric_mappings').select('rubric_id, game_id');
      if (error) {
        throw error;
      }
      ...
      if (!rubricGameMappings || rubricGameMappings.length === 0) {
        ...
        return new Response(JSON.stringify({
          success: true,
          message: `No rubric-game mappings found for config: ${configId}`,
          config: {
            config_id: configId,
            product_id: projectConfig.product_id,
            campaign_id: projectConfig.campaign_id
      ...
      return new Response(JSON.stringify({
        success: false,
        error: `Error fetching rubric-game mappings: ${err.message}`
      }), {
        status: 500,
        headers: {
          "Content-Type": "application/json",
    ...
    for (const mapping of rubricGameMappings){
      try {
        ...
        const { count: deletedCount, error: deleteError } = await supabase.from('game_contents').delete({
          count: 'exact'
        ...
        console.log(`[Galaxy Sync] Calling Galaxy API for rubric ${mapping.rubric_id}`);
        let response;
        try {
        ...
        if (gameContentDataArray.length > 0) {
          ...
          try {
            const { error: batchUpsertError } = await supabase.from('game_contents').upsert(gameContentDataArray, {
    ...
    console.log(`[Galaxy Sync] ========================================`);
    console.log(`[Galaxy Sync] Synchronization completed for config: ${configId}`);
    console.log(`[Galaxy Sync] - Synced: ${totalSynced}`);
    console.log(`[Galaxy Sync] - Errors: ${totalErrors}`);
    console.log(`[Galaxy Sync] - Deleted: ${totalDeleted}`);
    console.log(`[Galaxy Sync] ========================================`);
    return new Response(JSON.stringify({
      success: true,
      message: `Galaxy content synchronization completed for config: ${configId}`,
      config: {
        config_id: configId,
        product_id: projectConfig.product_id,
        campaign_id: projectConfig.campaign_id
