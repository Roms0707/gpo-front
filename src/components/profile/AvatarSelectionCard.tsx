import React from 'react';
import { Lock, Check } from 'lucide-react';
import type { ProfileAvatarWithUnlockStatus, ItemRarity, AvatarGameAffinity } from '../../types';
import CustomizationCardBorder from './CustomizationCardBorder';

interface AvatarSelectionCardProps {
  avatar: ProfileAvatarWithUnlockStatus;
  isSelected: boolean;
  onClick: () => void;
  userXp?: number;
  themeColor?: string;
  gameAffinityIcon?: React.ReactNode;
}

const getRarityColor = (rarity: ItemRarity): string => {
  switch (rarity) {
    case 'common': return '#9ca3af';
    case 'rare': return '#3b82f6';
    case 'epic': return '#a855f7';
    case 'legendary': return '#f97316';
    default: return '#9ca3af';
  }
};

const getRarityGlow = (rarity: ItemRarity): string => {
  switch (rarity) {
    case 'common': return 'shadow-gray-500/20';
    case 'rare': return 'shadow-blue-500/30';
    case 'epic': return 'shadow-purple-500/40';
    case 'legendary': return 'shadow-orange-500/50';
    default: return 'shadow-gray-500/20';
  }
};

export const getGameAffinityColor = (affinity: AvatarGameAffinity): string => {
  switch (affinity) {
    case 'lol': return '#c89b3c';
    case 'warzone': return '#4a5a3d';
    case 'valorant': return '#ff4655';
    case 'cs2': return '#ffb03b';
    case 'apex': return '#da291c';
    case 'rocket_league': return '#00b4ff';
    case 'universal': return '#8b5cf6';
    default: return '#6b7280';
  }
};

export const getGameAffinityName = (affinity: AvatarGameAffinity): string => {
  switch (affinity) {
    case 'lol': return 'League of Legends';
    case 'warzone': return 'Warzone';
    case 'valorant': return 'Valorant';
    case 'cs2': return 'Counter-Strike 2';
    case 'apex': return 'Apex Legends';
    case 'rocket_league': return 'Rocket League';
    case 'universal': return 'Elite Gaming';
    default: return 'Unknown';
  }
};

const AvatarSelectionCard: React.FC<AvatarSelectionCardProps> = ({
  avatar,
  isSelected,
  onClick,
  userXp = 0,
  themeColor,
  gameAffinityIcon,
}) => {
  const isLocked = !avatar.is_unlocked;
  const requiredXp = avatar.unlock_requirement?.xp || 0;
  const progress = requiredXp > 0 ? Math.min((userXp / requiredXp) * 100, 100) : 100;
  const gameColor = getGameAffinityColor(avatar.game_affinity);

  return (
    <CustomizationCardBorder
      rarity={avatar.rarity}
      isSelected={isSelected}
      isLocked={isLocked}
    >
      <button
        onClick={isLocked ? undefined : onClick}
        disabled={isLocked}
        className={`relative w-full p-2 rounded-xl border transition-all group ${
          isSelected
            ? 'border-transparent'
            : isLocked
            ? 'border-gray-800/30 cursor-not-allowed'
            : 'border-gray-700/30 hover:border-transparent'
        }`}
        style={{
          backgroundColor: isSelected ? `${themeColor || getRarityColor(avatar.rarity)}15` : undefined,
        }}
      >
        <div className="relative w-full aspect-square rounded-lg overflow-hidden mb-2">
          <img
            src={avatar.image_url}
            alt={avatar.name}
            className={`w-full h-full object-cover transition-all ${
              isLocked ? 'grayscale brightness-50' : ''
            } ${avatar.rarity === 'legendary' ? `shadow-lg ${getRarityGlow(avatar.rarity)}` : ''}`}
          />

          {avatar.rarity === 'legendary' && !isLocked && (
            <div className="absolute inset-0 bg-gradient-to-t from-orange-500/20 to-transparent animate-pulse pointer-events-none" />
          )}

          <div
            className="absolute inset-0 rounded-lg pointer-events-none"
            style={{
              boxShadow: `inset 0 0 0 2px ${isLocked ? '#374151' : getRarityColor(avatar.rarity)}40`,
            }}
          />

          {isLocked && (
            <div className="absolute inset-0 bg-dark-100/60 flex flex-col items-center justify-center">
              <Lock className="w-5 h-5 text-gray-500 mb-1" />
              {requiredXp > 0 && (
                <span className="text-[10px] text-gray-500 font-medium">
                  {requiredXp} XP
                </span>
              )}
            </div>
          )}

          {isSelected && !isLocked && (
            <div
              className="absolute top-1.5 right-1.5 w-5 h-5 rounded-full flex items-center justify-center shadow-lg"
              style={{ backgroundColor: themeColor || getRarityColor(avatar.rarity) }}
            >
              <Check className="w-3 h-3 text-white" />
            </div>
          )}

          {gameAffinityIcon && (
            <div
              className="absolute top-1.5 left-1.5 w-5 h-5 rounded flex items-center justify-center"
              style={{ backgroundColor: `${gameColor}40` }}
              title={getGameAffinityName(avatar.game_affinity)}
            >
              {gameAffinityIcon}
            </div>
          )}
        </div>

        <div className="text-left">
          <p className={`text-xs font-medium truncate ${isLocked ? 'text-gray-500' : 'text-white'}`}>
            {avatar.name}
          </p>
          <div className="flex items-center gap-1 mt-0.5">
            <span
              className="w-1.5 h-1.5 rounded-full"
              style={{ backgroundColor: isLocked ? '#6b7280' : getRarityColor(avatar.rarity) }}
            />
            <span className="text-[10px] text-gray-500 capitalize">{avatar.rarity}</span>
          </div>
        </div>

        {isLocked && requiredXp > 0 && (
          <div className="mt-1.5">
            <div className="h-1 bg-dark-300 rounded-full overflow-hidden">
              <div
                className="h-full bg-gradient-to-r from-blue-500 to-blue-400 transition-all"
                style={{ width: `${progress}%` }}
              />
            </div>
            <div className="flex items-center justify-between mt-0.5">
              <span className="text-[9px] text-gray-500">{userXp}/{requiredXp}</span>
              <span className="text-[9px] text-gray-600">{Math.round(progress)}%</span>
            </div>
          </div>
        )}
      </button>
    </CustomizationCardBorder>
  );
};

export default AvatarSelectionCard;
