...
const PerformanceContextCard: React.FC<PerformanceContextCardProps> = ({
  ...
  return (
    <div className="bg-white dark:bg-dark-200/50 rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden">
      <div
      ...
      <div className="p-4 space-y-4">
        {riotId && (
          <div className="text-center pb-3 border-b border-gray-200 dark:border-gray-800">
            ...
            <p className="text-gray-900 dark:text-white font-bold">{riotId}</p>
          </div>
        )}
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="text-center">
            ...
            <p className="text-lg font-bold text-gray-900 dark:text-white">{stats.winRate.toFixed(1)}%</p>
            <p className="text-xs text-gray-400 dark:text-gray-500">{t('coaching.winRate')}</p>
          </div>
          ...
          <div className="text-center">
            ...
            <p className="text-lg font-bold text-gray-900 dark:text-white">{stats.avgKDA.toFixed(2)}</p>
            <p className="text-xs text-gray-400 dark:text-gray-500">{t('coaching.avgKDA')}</p>
          </div>
          ...
          <div className="text-center">
            ...
            <p className="text-lg font-bold text-gray-900 dark:text-white">{stats.avgCSPerMin.toFixed(1)}</p>
            <p className="text-xs text-gray-400 dark:text-gray-500">{t('coaching.csPerMin')}</p>
          </div>
        </div>

        <div className="flex items-center justify-between pt-3 border-t border-gray-200 dark:border-gray-800">
          <div>
            <p className="text-xs text-gray-400 dark:text-gray-500">{t('coaching.last10Games')}</p>
        ...
        {stats.mostPlayedChampions.length > 0 && (
          <div className="pt-3 border-t border-gray-200 dark:border-gray-800">
            <p className="text-xs text-gray-400 dark:text-gray-500 mb-2">{t('coaching.mostPlayed')}</p>
            <div className="flex flex-wrap gap-2">
              {stats.mostPlayedChampions.slice(0, 3).map((champ) => (
                <div
                  key={champ.name}
                  className="px-2 py-1 rounded-md bg-gray-100 dark:bg-dark-300 text-xs"
                >
                  <span className="text-gray-900 dark:text-white">{champ.name}</span>
