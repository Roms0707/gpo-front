...
import { Zap, RotateCcw, Trophy, TrendingUp, Medal, User, Crown } from 'lucide-react';
...
const ReactionTimeGame: React.FC<ReactionTimeGameProps> = ({ gameName, gameId }) => {
  ...
  useEffect(() => {
    const savedStats = localStorage.getItem(`reactionTime_${gameId}`);
    if (savedStats) {
    ...
    try {
      ...
      if (historyData && historyData.length > 0) {
        ...
        if (totalGames >= 10) {
          ...
          improvement = Math.round(((previousAvg - recentAvg) / previousAvg) * 100);
        }
  ...
  const handleGameComplete = (score: number) => {
    const currentScores = [...(gameStats.gamesPlayed > 0 ? [gameStats.averageScore * gameStats.gamesPlayed] : []), score];
    const newAverage = Math.round(currentScores.reduce((a, b) => a + b, 0) / currentScores.length);

    const newStats = {
      lastScore: score,
      bestScore: gameStats.gamesPlayed > 0 ? Math.min(gameStats.bestScore, score) : score,
      averageScore: newAverage,
      gamesPlayed: gameStats.gamesPlayed + 1
    };

    setGameStats(newStats);
    localStorage.setItem(`reactionTime_${gameId}`, JSON.stringify(newStats));
  ...
  const saveScoreToDatabase = async (score: number) => {
    try {
      ...
      if (error) {
        console.error('Error saving reaction time score:', error);
        return;
      }

      console.log('Reaction time score saved successfully:', score);
    } catch (error) {
      console.error('Error saving reaction time score:', error);
    }
  };
  ...
  const resetStats = () => {
    ...
    localStorage.removeItem(`reactionTime_${gameId}`);
  };

  const getRatingColor = (score: number) => {
    if (score < 200) return 'text-green-500';
    if (score < 250) return 'text-blue-500';
  ...
  return (
    <div className="bg-white dark:bg-dark-100 rounded-xl overflow-hidden border border-gray-200 dark:border-gray-800">
      {/* Header */}
      <div className="bg-gradient-to-r from-yellow-500/20 to-orange-500/20 p-4 border-b border-gray-200 dark:border-gray-800">
          ...
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600 dark:text-gray-400">
                {t('gaming.improveYourReactionFor', { game: gameName })}
              </p>
            </div>
          </div>
        ...
        <div className="p-4 bg-gray-50 dark:bg-dark-200/50 border-b border-gray-200 dark:border-gray-800">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="text-center">
              <div className={`text-lg font-bold ${getRatingColor(gameStats.lastScore)}`}>
                {gameStats.lastScore} {t('gaming.milliseconds')}
              </div>
              <div className="text-xs text-gray-600 dark:text-gray-400">{t('gaming.lastScore')}</div>
            </div>
            <div className="text-center">
              <div className={`text-lg font-bold ${getRatingColor(gameStats.bestScore)}`}>
                {gameStats.bestScore} {t('gaming.milliseconds')}
              </div>
              <div className="text-xs text-gray-600 dark:text-gray-400">{t('gaming.fastestReaction')}</div>
            </div>
            <div className="text-center">
              <div className={`text-lg font-bold ${getRatingColor(gameStats.averageScore)}`}>
                {gameStats.averageScore} {t('gaming.milliseconds')}
              </div>
              <div className="text-xs text-gray-600 dark:text-gray-400">{t('gaming.averageReactionTime')}</div>
            </div>
            <div className="text-center">
          ...
          {isLoadingPersonalStats ? (
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                ...
                <div className="text-center p-3 bg-white dark:bg-dark-200 rounded-lg">
                  <div className={`text-xl font-bold ${getRatingColor(personalStats.averageScore)}`}>
                    {personalStats.averageScore} {t('gaming.milliseconds')}
                  </div>
                  <div className="text-xs text-gray-600 dark:text-gray-400">{t('gaming.averageScore')}</div>
                </div>
                <div className="text-center p-3 bg-white dark:bg-dark-200 rounded-lg">
                  <div className={`text-xl font-bold ${getRatingColor(personalStats.bestScore)}`}>
                    {personalStats.bestScore} {t('gaming.milliseconds')}
                  </div>
                      ...
                      <div className="flex items-center">
                        ...
                        <div>
                          <div className={`font-medium ${
                            ...
                            {entry.score} {t('gaming.milliseconds')}
                            {entry.score === personalStats.bestScore && (
        ...
        <div className="p-4 bg-gray-50 dark:bg-dark-200/50 border-b border-gray-200 dark:border-gray-800">
          ...
          {isLoadingLeaderboard ? (
              ...
              {leaderboard.map((entry, index) => (
                ...
                >
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-xs text-gray-500 dark:text-gray-400">{t('gaming.milliseconds')}</div>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="text-center py-8">
              ...
              <p className="text-sm text-gray-500 dark:text-gray-500 mt-1">
                {t('gaming.beTheFirstToSetRecord')}
              </p>
            </div>
          )}
        </div>
        ...
        {showTipsOverlay && (
          <div className="absolute top-4 right-4 bg-black/70 text-white p-3 rounded-lg text-sm max-w-xs transition-opacity duration-300">
            ...
            <ul className="text-xs space-y-1">
              <li>• {t('gaming.tipWaitForGreen')}</li>
              <li>• {t('gaming.tipClickAsFastAsYou')}</li>
              <li>• {t('gaming.tipDontClickTooEarly')}</li>
              <li>• {t('gaming.tipPracticeImproves')}</li>
            </ul>
          </div>
        )}
      </div>
